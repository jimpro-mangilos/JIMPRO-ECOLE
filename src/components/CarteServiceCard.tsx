...
// ═══════════════════════════════════════════════════════════════════════════════
// Carte de service PAYSAGE (86 × 54 mm) — même style Prestige, orientation horizontale
// ═══════════════════════════════════════════════════════════════════════════════
export const CARTE_SERVICE_LAND_W = 516; // 86 mm × 6
export const CARTE_SERVICE_LAND_H = 324; // 54 mm × 6

export function CarteServiceCardLandscape({ personnel, schoolName, logoUrl, qrDataUrl }: {
  personnel: CarteService;
  schoolName: string;
  logoUrl: string | null;
  qrDataUrl: string;
  ...
  // Entête diagonale (paysage)
  const headerSvg = `data:image/svg+xml;utf8,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="516" height="62" viewBox="0 0 516 62">` +
    `<defs>` +
    `<linearGradient id="lh" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${GREEN_DEEP}"/><stop offset="0.6" stop-color="${GREEN}"/><stop offset="1" stop-color="${GREEN_LIGHT}"/></linearGradient>` +
    `<linearGradient id="lg" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${GOLD_LIGHT}"/><stop offset="0.55" stop-color="${GOLD}"/><stop offset="1" stop-color="${GOLD_DARK}"/></linearGradient>` +
    `</defs>` +
    `<rect x="0" y="0" width="516" height="62" fill="url(#lh)"/>` +
    `<polygon points="0,50 516,36 516,62 0,62" fill="url(#lg)" opacity="0.95"/>` +
    `<polygon points="0,44 516,30 516,34 0,48" fill="#ffffff" opacity="0.10"/>` +
    `</svg>`
  )}`;

  // Bande inférieure : 3 triangles (paysage)
  const footerSvg = `data:image/svg+xml;utf8,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="516" height="52" viewBox="0 0 516 52" preserveAspectRatio="none">` +
    `<defs>` +
    `<linearGradient id="lf" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${GREEN_LIGHT}"/><stop offset="0.5" stop-color="${GREEN}"/><stop offset="1" stop-color="${GREEN_DEEP}"/></linearGradient>` +
    `<linearGradient id="lfo" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${GOLD_LIGHT}"/><stop offset="0.55" stop-color="${GOLD}"/><stop offset="1" stop-color="${GOLD_DARK}"/></linearGradient>` +
    `</defs>` +
    `<polygon points="0,52 516,52 516,26 466,10 330,26 215,3 100,26 50,10 0,26" fill="url(#lf)"/>` +
    `<polygon points="0,26 50,10 100,26" fill="url(#lfo)"/>` +
    `<polygon points="330,26 466,10 516,26" fill="url(#lfo)"/>` +
    `</svg>`
  )}`;

  return (
    <div style={{ width: CARTE_SERVICE_LAND_W, height: CARTE_SERVICE_LAND_H, position: 'relative', overflow: 'hidden', fontFamily: FONT, background: IVORY, borderRadius: 10, color: NAVY, boxShadow: '0 16px 40px rgba(11,61,46,0.20)' }}>
      {/* Cadre intérieur */}
      <div style={{ position: 'absolute', top: 5, left: 5, right: 5, bottom: 5, borderRadius: 7, border: '1px solid rgba(201,162,39,0.30)' }} />

      {/* Entête */}
      <img src={headerSvg} alt="" style={{ position: 'absolute', top: 0, left: 0, width: 516, height: 62 }} />

      {/* Logo école */}
      <div style={{ position: 'absolute', top: 8, left: 20, width: 46, height: 46, borderRadius: '50%', background: '#ffffff', border: `2px solid ${GOLD}`, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 3px 10px rgba(10,53,41,0.30)' }}>
        {logoUrl && !logoError ? (
          <img src={logoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={() => setLogoError(true)} />
        ) : (
          <span style={{ fontSize: 14, fontWeight: 800, color: GREEN }}>{getSchoolInitials(schoolName)}</span>
        )}
      </div>

      {/* Nom de l'école + titre */}
      <div style={{ position: 'absolute', top: 12, left: 78, right: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ width: 4, height: 4, transform: 'rotate(45deg)', background: GOLD_LIGHT, flexShrink: 0 }} />
          <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: 1.5, color: '#ffffff', textTransform: 'uppercase', marginLeft: 8, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{schoolName}</span>
        </div>
        <div style={{ fontSize: 9, fontWeight: 600, letterSpacing: 3.5, color: GOLD_LIGHT, marginTop: 3, textTransform: 'uppercase' }}>Carte de service</div>
      </div>

      {/* Photo ronde */}
      <div style={{ position: 'absolute', top: 78, left: 30, width: 108, height: 108, borderRadius: '50%', background: `linear-gradient(140deg, ${GOLD_LIGHT}, ${GOLD}, ${GOLD_DARK})`, padding: 2, boxShadow: '0 0 0 4px rgba(255,255,255,0.92), 0 10px 22px rgba(11,61,46,0.25)' }}>
        <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: '#ffffff', padding: 3 }}>
          <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden', background: '#f6efe0' }}>
            {personnel.photo_url && !photoError ? (
              <img src={personnel.photo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={() => setPhotoError(true)} />
            ) : (
              <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, fontWeight: 800, color: GOLD_DARK }}>{initials}</div>
            )}
          </div>
        </div>
      </div>

      {/* Identité + champs (à droite de la photo) */}
      <div style={{ position: 'absolute', top: 80, left: 156, right: 132 }}>
        <div style={{ fontSize: 21, fontWeight: 800, color: NAVY, letterSpacing: 0.3, lineHeight: 1.15 }}>
          {personnel.nom.toUpperCase()} <span style={{ fontWeight: 700, color: NAVY }}>{personnel.prenom}</span>
        </div>
        <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: 1.4, color: GOLD_DARK, textTransform: 'uppercase', marginTop: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{personnel.fonction}</div>
      </div>

      {/* Champs */}
      <div style={{ position: 'absolute', top: 148, left: 156, right: 132 }}>
        <Field label="Matricule" value={personnel.matricule || '—'} nowrap />
        <Field label="Téléphone" value={personnel.telephone || '—'} nowrap />
        <Field label="Année scolaire" value="2026-2027" />
      </div>

      {/* QR + JIMPRO SCAN */}
      <div style={{ position: 'absolute', top: 78, right: 16, zIndex: 4 }}>
        <div style={{ borderRadius: 12, background: `linear-gradient(140deg, ${GOLD_LIGHT}, ${GOLD}, ${GOLD_DARK})`, padding: 2, boxShadow: '0 8px 18px rgba(11,61,46,0.16)' }}>
          <div style={{ borderRadius: 10, background: '#ffffff', overflow: 'hidden' }}>
            <div style={{ background: `linear-gradient(90deg, ${GOLD_LIGHT}, ${GOLD}, ${GOLD_DARK})`, textAlign: 'center', padding: '2px 0', fontSize: 9.5, fontWeight: 800, letterSpacing: 1.5, color: '#06291e', textTransform: 'uppercase' }}>JIMPRO SCAN</div>
            <div style={{ padding: 3 }}>
              <img src={qrDataUrl} alt="" style={{ width: 106, height: 106, display: 'block' }} />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Bande inférieure */}
      <img src={footerSvg} alt="" style={{ position: 'absolute', bottom: 0, left: 0, width: 516, height: 52 }} />
      <div style={{ position: 'absolute', bottom: 12, left: 60, right: 60, textAlign: 'center', fontSize: 12.5, fontWeight: 700, letterSpacing: 1, color: '#ffffff', textTransform: 'lowercase', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{siteWeb}</div>
    </div>
  );
}
