            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {eleves.map(eleve => (
                <tr key={eleve.id} className="hover:bg-gray-50 transition-colors">
                  {isItManager() && <td className="px-4 py-3"><input type="checkbox" checked={selectedIds.has(eleve.id)} onChange={() => toggleSelectOne(eleve.id)} className="rounded" /></td>}
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {!isReadOnly() && <button onClick={() => handleEditClick(eleve)} className="p-2 rounded-lg hover:bg-amber-50 text-amber-600" title="Modifier"><Edit className="w-4 h-4" /></button>}
                      {!isReadOnly() && <button onClick={() => handlePayment(eleve)} className="p-2 rounded-lg hover:bg-green-50 text-green-600" title="Paiement"><Plus className="w-4 h-4" /></button>}
                      {isItManager() && <button onClick={() => deleteEleve(eleve.id)} className="p-2 rounded-lg hover:bg-red-50 text-red-500" title="Supprimer"><Trash2 className="w-4 h-4" /></button>}
                    </div>
                  </td>
                </tr>
              ))}
