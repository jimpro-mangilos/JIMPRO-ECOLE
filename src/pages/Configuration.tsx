...
export default function Configuration() {
  ...
  const [showTypeUniformeForm, setShowTypeUniformeForm] = useState(false); const [editingTypeUniforme, setEditingTypeUniforme] = useState<any>(null); const [typeUniformeForm, setTypeUniformeForm] = useState({ libelle: '', description: '', is_active: true });
  ...
  return (
    <div className="space-y-6">
      ...
      {error && <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm"><AlertCircle className="w-4 h-4" />{error}<button onClick={() => setError('')} className="ml-auto"><X className="w-4 h-4" /></button></div>}
      {success && <div className="flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm"><Check className="w-4 h-4" />{success}</div>}
      ...
      {activeTab === 'logo' && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-lg font-bold mb-4">Logo de l'établissement</h2>
          <div className="flex items-center gap-6">
            <img src={logoUrl} alt="Logo" className="w-32 h-32 object-contain border rounded-xl bg-gray-50" />
            <div>
      ...
      {(activeTab === 'sections' || activeTab === 'options' || activeTab === 'classes' || activeTab === 'motifs' || activeTab === 'types_paiement' || activeTab === 'annees_scolaires' || activeTab === 'prefixes_matricule' || activeTab === 'types_uniforme') && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {canManageConfiguration() && (
              <button onClick={() => {
                ...
                else if (activeTab === 'classes') { setEditingClasse(null); setClasseForm(emptyForm({ section_id: '', option_id: '', niveau: '', designation: '' })); setShowClasseForm(true); }
                ...
                else if (activeTab === 'types_uniforme') { setEditingTypeUniforme(null); setTypeUniformeForm({ libelle: '', description: '', is_active: true }); setShowTypeUniformeForm(true); }
          ...
          <table className="w-full"><thead><tr className="border-b text-left text-xs font-semibold text-gray-600 uppercase">
            {activeTab === 'sections' && <><th className="px-3 py-2">Nom</th><th className="px-3 py-2">Description</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
            {activeTab === 'options' && <><th className="px-3 py-2">Nom</th><th className="px-3 py-2">Section</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
            {activeTab === 'classes' && <><th className="px-3 py-2">Nom</th><th className="px-3 py-2">Niveau</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
            {activeTab === 'motifs' && <><th className="px-3 py-2">Libellé</th><th className="px-3 py-2">Description</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
            {activeTab === 'types_paiement' && <><th className="px-3 py-2">Libellé</th><th className="px-3 py-2">Description</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
            {activeTab === 'annees_scolaires' && <><th className="px-3 py-2">Année</th><th className="px-3 py-2">Début</th><th className="px-3 py-2">Fin</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
            {activeTab === 'prefixes_matricule' && <><th className="px-3 py-2">Section</th><th className="px-3 py-2">Libellé</th><th className="px-3 py-2">Préfixe</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
            {activeTab === 'types_uniforme' && <><th className="px-3 py-2">Libellé</th><th className="px-3 py-2">Description</th><th className="px-3 py-2">Actif</th><th className="px-3 py-2 text-right">Actions</th></>}
          </tr></thead><tbody className="divide-y">
            {activeTab === 'sections' && sections.map((s: any) => <tr key={s.id}><td className="px-3 py-2 text-sm font-medium">{s.nom}</td><td className="px-3 py-2 text-sm text-gray-500">{s.description}</td><td className="px-3 py-2">{s.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingSection(s); setSectionForm({ nom: s.nom, description: s.description, is_active: s.is_active }); setShowSectionForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteSection, s.id, 'Section')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
            {activeTab === 'options' && options.map((o: any) => <tr key={o.id}><td className="px-3 py-2 text-sm font-medium">{o.nom}</td><td className="px-3 py-2 text-sm text-gray-500">{sections.find((s: any) => s.id === o.section_id)?.nom}</td><td className="px-3 py-2">{o.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingOption(o); setOptionForm({ nom: o.nom, section_id: o.section_id, description: o.description, is_active: o.is_active }); setShowOptionForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteOption, o.id, "Option")} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
            {activeTab === 'classes' && classes.map((c: any) => <tr key={c.id}><td className="px-3 py-2 text-sm font-medium">{c.nom}</td><td className="px-3 py-2 text-sm text-gray-500">{c.niveau || '—'}</td><td className="px-3 py-2">{c.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingClasse(c); setClasseForm({ nom: c.nom, section_id: c.section_id, option_id: c.option_id || '', niveau: c.niveau || '', designation: c.designation || '', description: c.description, is_active: c.is_active }); setShowClasseForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteClasse, c.id, 'Classe')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
            {activeTab === 'motifs' && motifs.map((m: any) => <tr key={m.id}><td className="px-3 py-2 text-sm font-medium">{m.libelle}</td><td className="px-3 py-2 text-sm text-gray-500">{m.description}</td><td className="px-3 py-2">{m.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingMotif(m); setMotifForm({ libelle: m.libelle, description: m.description, is_active: m.is_active }); setShowMotifForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteMotif, m.id, 'Motif')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
            {activeTab === 'types_paiement' && typesPaiement.map((t: any) => <tr key={t.id}><td className="px-3 py-2 text-sm font-medium">{t.libelle}</td><td className="px-3 py-2 text-sm text-gray-500">{t.description}</td><td className="px-3 py-2">{t.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingTypePaiement(t); setTypePaiementForm({ libelle: t.libelle, description: t.description, is_active: t.is_active }); setShowTypePaiementForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteTypePaiement, t.id, 'Type')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
            {activeTab === 'annees_scolaires' && anneeScolaires.map((a: any) => <tr key={a.id}><td className="px-3 py-2 text-sm font-medium">{a.annee}</td><td className="px-3 py-2 text-sm text-gray-500">{a.date_debut || '—'}</td><td className="px-3 py-2 text-sm text-gray-500">{a.date_fin || '—'}</td><td className="px-3 py-2">{a.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingAnneeScolaire(a); setAnneeScolaireForm({ annee: a.annee, date_debut: a.date_debut || '', date_fin: a.date_fin || '', is_active: a.is_active }); setShowAnneeScolaireForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteAnneeScolaire, a.id, 'Année')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
            {activeTab === 'prefixes_matricule' && sectionPrefixes.map((p: any) => <tr key={p.id}><td className="px-3 py-2 text-sm font-medium">{p.section}</td><td className="px-3 py-2 text-sm text-gray-500">{p.libelle}</td><td className="px-3 py-2 text-sm"><code className="bg-gray-100 px-2 py-0.5 rounded">{p.prefix}</code></td><td className="px-3 py-2">{p.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingPrefix(p); setPrefixForm({ section: p.section, libelle: p.libelle, prefix: p.prefix, is_active: p.is_active }); setShowPrefixForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deletePrefix, p.id, 'Préfixe')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
            {activeTab === 'types_uniforme' && typesUniforme.map((u: any) => <tr key={u.id}><td className="px-3 py-2 text-sm font-medium">{u.libelle}</td><td className="px-3 py-2 text-sm text-gray-500">{u.description}</td><td className="px-3 py-2">{u.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-3 py-2 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingTypeUniforme(u); setTypeUniformeForm({ libelle: u.libelle, description: u.description, is_active: u.is_active }); setShowTypeUniformeForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteTypeUniforme, u.id, 'Type uniforme')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
          </tbody></table>
        </div>
      )}
      ...
      {showOptionForm && <Modal title={editingOption ? "Modifier Option" : "Nouvelle Option"} onClose={() => setShowOptionForm(false)} onSubmit={e => { e.preventDefault(); handleSubmit(upsertOption, optionForm, setShowOptionForm, () => setOptionForm(emptyForm({ section_id: '' })), 'Option', !!editingOption, { id: editingOption?.id, maxOrdre: Math.max(0, ...options.map((o: any) => o.ordre)) }); }}>
        ...
        <div><label className="block text-sm font-medium mb-1">Section</label><select value={optionForm.section_id} onChange={e => setOptionForm(p => ({ ...p, section_id: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm"><option value="">—</option>{sections.map((s: any) => <option key={s.id} value={s.id}>{s.nom}</option>)}</select></div>
      ...
      {showClasseForm && <Modal title={editingClasse ? 'Modifier Classe' : 'Nouvelle Classe'} onClose={() => setShowClasseForm(false)} onSubmit={e => { e.preventDefault(); const maxO = Math.max(0, ...classes.map((c: any) => c.ordre)); handleSubmit(upsertClasse, classeForm, setShowClasseForm, () => setClasseForm(emptyForm({ section_id: '', option_id: '', niveau: '', designation: '' })), 'Classe', !!editingClasse, { id: editingClasse?.id, maxOrdre: maxO }); }}>
        ...
        <div><label className="block text-sm font-medium mb-1">Section</label><select value={classeForm.section_id} onChange={e => setClasseForm(p => ({ ...p, section_id: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm"><option value="">—</option>{sections.map((s: any) => <option key={s.id} value={s.id}>{s.nom}</option>)}</select></div>
      ...
      {showPrefixForm && <Modal title={editingPrefix ? 'Modifier Préfixe' : 'Nouveau Préfixe'} onClose={() => setShowPrefixForm(false)} onSubmit={async e => { e.preventDefault(); clearMsg(); try { await upsertPrefix(prefixForm, editingPrefix?.id); showSuccess('Préfixe enregistré'); setPrefixForm({ section: '', libelle: '', prefix: '', is_active: true }); setEditingPrefix(null); setShowPrefixForm(false); invalidatePrefixCache(); } catch { showError('Erreur'); } }}>
        <div><label className="block text-sm font-medium mb-1">Section</label><select value={prefixForm.section} onChange={e => setPrefixForm(p => ({ ...p, section: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm"><option value="">—</option>{sections.map((s: any) => <option key={s.id} value={s.nom}>{s.nom}</option>)}</select></div>
      ...
      {/* Type Uniforme Modal */}
      {showTypeUniformeForm && <Modal title={editingTypeUniforme ? "Modifier" : "Nouveau Type Uniforme"} onClose={() => setShowTypeUniformeForm(false)} onSubmit={async e => { e.preventDefault(); clearMsg(); try { await upsertTypeUniforme(typeUniformeForm, editingTypeUniforme?.id); showSuccess('Enregistré'); setTypeUniformeForm({ libelle: '', description: '', is_active: true }); setEditingTypeUniforme(null); setShowTypeUniformeForm(false); } catch { showError('Erreur'); } }}>
...
function Field({ label, value, onChange, type = 'text' }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return <div><label className="block text-sm font-medium text-gray-700 mb-1">{label}</label><input type={type} value={value} onChange={e => onChange(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" /></div>;
}
