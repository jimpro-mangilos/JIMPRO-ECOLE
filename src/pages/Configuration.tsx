  ...
  return (
    <div className="space-y-4">
      ...
      {(activeTab === 'sections' || activeTab === 'options' || activeTab === 'classes' || activeTab === 'motifs' || activeTab === 'types_paiement' || activeTab === 'annees_scolaires' || activeTab === 'prefixes_matricule' || activeTab === 'types_uniforme') && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <table className="w-full"><thead><tr className="border-b text-left text-xs font-semibold text-gray-600 uppercase">
            ...
            {activeTab === 'types_uniforme' && typesUniforme.map((u: any) => <tr key={u.id}><td className="px-2 py-1.5 text-sm font-medium">{u.libelle}</td><td className="px-2 py-1.5 text-sm text-gray-500">{u.description}</td><td className="px-2 py-1.5">{u.is_active ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-red-400" />}</td><td className="px-2 py-1.5 text-right">{canManageConfiguration() && <><button onClick={() => { setEditingTypeUniforme(u); setTypeUniformeForm({ libelle: u.libelle, description: u.description, is_active: u.is_active }); setShowTypeUniformeForm(true); }} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDelete(deleteTypeUniforme, u.id, 'Type uniforme')} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button></>}</td></tr>)}
          </tbody></table>
        </div>
      )}
      ...
      {showClasseForm && <Modal title={editingClasse ? 'Modifier Classe' : 'Nouvelle Classe'} onClose={() => setShowClasseForm(false)} onSubmit={e => { e.preventDefault(); const maxO = Math.max(0, ...classes.map((c: any) => c.ordre)); handleSubmit(upsertClasse, classeForm, setShowClasseForm, () => setClasseForm(emptyForm({ section_id: '', option_id: '', niveau: '', designation: '' })), 'Classe', !!editingClasse, { id: editingClasse?.id, maxOrdre: maxO }); }}>
        ...
        <div><label className="block text-sm font-medium mb-1">Section</label><select value={classeForm.section_id} onChange={e => { setClasseForm((p: any) => ({ ...p, section_id: e.target.value, option_id: '' })); }} className="w-full px-2 py-1.5 border rounded-lg text-sm"><option value="">—</option>{sections.map((s: any) => <option key={s.id} value={s.id}>{s.nom}</option>)}</select></div>
        <div><label className="block text-sm font-medium mb-1">Option</label><select value={classeForm.option_id} onChange={e => setClasseForm((p: any) => ({ ...p, option_id: e.target.value }))} className="w-full px-2 py-1.5 border rounded-lg text-sm"><option value="">—</option>{options.filter((o: any) => !classeForm.section_id || o.section_id === classeForm.section_id).map((o: any) => <option key={o.id} value={o.id}>{o.nom}</option>)}</select></div>
